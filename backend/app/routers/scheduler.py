...
from app.schemas import SchedulerStatus, MessageResponseGeneric, LogResponse
...
async def get_scheduler_status(
    ...
    try:
        ...
        status = SchedulerStatus(
            ...
            total_messages_sent=job_status.get("total_messages_sent", 0) if job_status else recent_logs,
...
async def get_scheduler_logs(
    ...
    status_filter: Optional[str] = Query(None, description="Filter by status: success, failed, blacklisted"),
...
async def get_log_stats(
    ...
    try:
        ...
        total = db.query(Log).filter(
            Log.user_id == current_user.id,
            Log.created_at >= since
        ).count()
        
        success = db.query(Log).filter(
            Log.user_id == current_user.id,
            Log.created_at >= since,
            Log.status == 'success'
        ).count()
        
        failed = db.query(Log).filter(
            Log.user_id == current_user.id,
            Log.created_at >= since,
            Log.status == 'failed'
        ).count()
        
        blacklisted = db.query(Log).filter(
            Log.user_id == current_user.id,
            Log.created_at >= since,
